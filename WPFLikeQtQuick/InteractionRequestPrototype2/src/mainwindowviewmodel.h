...
public slots:
    void open()
    {
        qDebug() << __func__;
        static int count = 0;
        auto&& context = std::make_shared<Confirmation>( "たいとる", QString("こんてんつ %1").arg( count ) );
        context->setAcceptedAction( [context]
        {
            qDebug() << "Accepted:" << "result:" << context->resultCode();
            ++count;
        } );
        context->setRejectedAction( [context]
        {
            qDebug() << "Rejected:" << "result:" << context->resultCode();
        } );
